...
def main():
    ...
    urls = [f"./shards/shared_0{i}.tar" for i in range(1, 10)]
    # urls.extend(f"./shards/shared_{i}.tar" for i in range(10, 21))
    for url in urls:
        if not os.path.exists(url):
            raise FileNotFoundError(f"Dataset shard not found: {url}")
